import { PitcherStats } from "../pitchingStats";
import { PitcherSeasonStats } from "../../types/PlayerSeasonStats";

export const calculatePitchingSeasonStats = (gameStatsList: PitcherStats[]): PitcherSeasonStats => {
  const totalStats: PitcherSeasonStats = {
  ...
  gameStatsList.forEach(stats => {
    ...
    if (stats.winLoss === 'win') totalStats.wins++;
    if (stats.winLoss === 'loss') totalStats.losses++;
  });
